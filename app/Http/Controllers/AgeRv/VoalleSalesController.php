...
class VoalleSalesController extends Controller
{
    ...
    public function create()
    {
        $query = 'SELECT DISTINCT
                    ...
                    ac.user as "conexao",
                    c.amount AS "valor",
                    (SELECT id AS vendedor_id FROM erp.people p WHERE c.seller_1_id = p.id),
                    (SELECT name AS "vendedor" FROM erp.people p WHERE c.seller_1_id = p.id),
                    (SELECT id AS supervisor_id FROM erp.people p WHERE c.seller_2_id = p.id),
                ...
                foreach($salesVoalle as $sale => $value) {
                    $dataVoalle->create([
                        ...
                        'valor' => $value->valor,
                        'id_vendedor' => $value->vendedor_id,
                        'vendedor' => $value->vendedor,
                        'id_supervisor' => $value->supervisor_id,
